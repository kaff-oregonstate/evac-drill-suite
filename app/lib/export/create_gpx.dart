...
class CreateGpx {
  Future<void> fromList(File file) async {
    ...
    /// DEPRECATED:
    // List<ParticipantLocation> trackedPoints = await databaseManager.getTrajectory();

    List<dynamic> trackedPoints = await databaseManager.getTrajectoryStrings();
    print(trackedPoints);
    ...
    for (var point in trackedPoints) {
      if (point != null && point.runtimeType == String) sink.write(point);
    }
    sink.write(footer);
    await sink.flush();
    sink.close();
